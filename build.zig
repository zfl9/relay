...
fn init_dep(step: *Step, dep: DependLib) void {
    ...
    if (!path_exists(dep.tarball))
        step.dependOn(download(dep.url, dep.tarball));

    step.dependOn(rm(dep.src_dir));

    step.dependOn(tar_extract(dep.tarball, "dep"));
}
...
/// step: log info
fn log(comptime format: []const u8, args: anytype) *Step {
    return &_b.addLog(format, args).step;
}

/// step: /bin/sh command
fn sh(sh_cmd: []const u8) *Step {
...
fn sh_x(sh_cmd: []const u8) *Step {
    return sh(fmt("set -x; {s}", .{sh_cmd}));
}

/// step: remove dir or file
fn rm(path: []const u8) *Step {
    return &_b.addRemoveDirTree(path).step;
}

/// step: download file
fn download(url: []const u8, path: []const u8) *Step {
    const cmd_ =
        \\  url='{s}'; path='{s}'
...
fn tar_extract(tarball_path: []const u8, to_dir: []const u8) *Step {
...
fn dupe(bytes: []const u8) []u8 {
    return _b.dupe(bytes);
}
...
fn gen_modules_zig() void {
    ...
    while (it.next() catch unreachable) |file| {
        ...
        if (!std.mem.endsWith(u8, file.name, ".zig"))
            continue;
        list.append(dupe(file.name[0 .. file.name.len - 4])) catch unreachable;
    }
...
fn new_exe(name: []const u8) *LibExeObjStep {
    const exe = _b.addExecutable(name, null);
    setup_artifact(exe);
    return exe;
}

fn new_obj(name: []const u8, root_src: ?[]const u8) *LibExeObjStep {
    const obj = _b.addObject(name, root_src);
    setup_artifact(obj);
    return obj;
}

fn setup_artifact(step: *LibExeObjStep) void {
...
fn build_wolfssl() *Step {
    ...
    wolfssl.dependOn(sh_x(cmd));

    return wolfssl;
}

fn add_app_objs(app_exe: *LibExeObjStep) void {
    ...
    while (it.next() catch unreachable) |file| {
        ...
        if (!is_root_zig and !is_c_file)
            continue;

        const filepath = fmt("src/{s}", .{file.name});

        const obj = new_obj(file.name, if (is_root_zig) filepath else null);
...
fn configure() void {
    ...
    const rm_cache = rm(_b.cache_root);
    const rm_wolfssl = rm(_dep_wolfssl.base_dir); // current target
    const rm_wolfssl_all = sh(fmt("rm -fr {s}@*", .{_dep_wolfssl.src_dir})); // all targets
